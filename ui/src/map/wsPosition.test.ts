...
describe("parseRoomPosition", () => {
  ...
  it("carries the yellow exit-diff arrays through", () => {
    const pos = parseRoomPosition({
      type: "room_position",
      confidence: "yellow",
      position_valid: true,
    ...
    expect(pos.exitsAddedLive).toEqual(["U"]);
    expect(pos.exitsRemovedMap).toEqual(["N"]);
  });
  ...
  it("normalizes an unknown/empty confidence to red", () => {
    ...
    expect(parseRoomPosition({ confidence: "yellow" }).confidence).toBe(
      "yellow",
    );
  });
});
