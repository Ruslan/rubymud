...
func (m *mockTimerControl) hasAnyAction() bool {
	return m.on != "" || m.off != "" || m.reset != "" || m.set != "" || m.size != ""
}
...
func TestTickerCommandDiagnostics(t *testing.T) {
	...
	tests := []struct {
		input    string
		expected string
		action   string // "on", "off", "reset", "set", "size" or ""
		val      float64
	}{
		{"#tickon", "", "on", 0},
		{"#tickon {junk}", "#tickon: usage: #tickon (accepts no arguments)", "", 0},
		{"#tickoff", "", "off", 0},
		{"#tickoff {junk}", "#tickoff: usage: #tickoff (accepts no arguments)", "", 0},
		{"#ticksize", "#ticksize: usage: #ticksize {seconds}", "", 0},
		{"#ticksize {10}", "", "size", 10},
		{"#ticksize {10} {junk}", "#ticksize: usage: #ticksize {seconds}", "", 0},
		{"#ticksize {-1}", "#ticksize: invalid non-negative seconds \"-1\"", "", 0},
		{"#tickset", "", "reset", 0},
		{"#tickset {45}", "", "set", 45},
		{"#tickset {45} {junk}", "#tickset: too many arguments, usage: #tickset [{seconds}]", "", 0},
		{"#tickset {-5}", "#tickset: invalid non-negative seconds \"-5\"", "", 0},
	}
