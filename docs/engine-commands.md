...
### Эхо локальных команд

Локальные системные команды (начинающиеся с `#`) ведут себя по-разному в зависимости от того, как они запущены:
...
- примеры настройки хоткеев доступны через Settings UI, а также в `.tt` файлах профилей в директории `config/`.
...
Команды `#showme`, `#show` и `#woutput` поддерживают HTML-подобную разметку для оформления текста. Эта разметка компилируется в ANSI-коды перед выводом и не влияет на входящий текст от MUD или другие правила обработки.

### Поддерживаемые теги стилей

- `<b>`, `<bold>` — жирный шрифт
- `<faint>` — блеклый шрифт
- `<i>`, `<italic>` — курсив
- `<u>`, `<underline>` — подчеркивание
- `<s>`, `<strike>`, `<strikethrough>` — зачеркивание
- `<blink>` — мигание
- `<reverse>` — инверсия цветов
- `<reset>` — сброс всех стилей (очищает стек оформления)

### Цвета

Вы можете использовать названия цветов напрямую в тегах:
- `<red>текст</red>`
- `<light-red>текст</light-red>` (имена с пробелами пишутся через дефис)
- `<bg-blue>фон</bg-blue>`

Или использовать явные формы для переднего плана (`fg`) и фона (`bg`):
- `<fg red>...</fg>`
- `<bg blue>...</bg>`
- `<fg #ff8800>...</fg>` (Hex RGB)
- `<fg 256:196>...</fg>` (ANSI 256)
- `<fg rgb255,128,0>...</fg>` (RGB компоненты)

### Особенности

- **Вложенность**: Стили можно вкладывать друг в друга. При закрытии тега восстанавливается оформление, которое было до его открытия.
  Пример: `<red>красный <b>жирный красный</b> снова красный</red>`
- **Безопасность**: Неизвестные или битые теги (например, `<foo>` или `<red`) не обрабатываются и выводятся как обычный текст.
- **Изоляция**: Разметка работает только в локальном выводе. Она не подменяет работу `#highlight` или `#sub`.

## `#if`

Условное выполнение команд на основе вычисления выражений.
...
```text
#if {$hp == 100} {#showme {Full HP!}}
#if {$target == ""} {#showme {No target!}} {kill $target}
#if {($a + $b) / 2 == 10} {say Average is 10}
#if {$hp > 20 && $mana >= 10} {cast 'heal'}
#alias {ден} {#if {%0 > 0} {положить %0 монет $bag} {положить $money монет $bag}}
#action {^A (.*) attacks you!$} {#if {%1 == "goblin"} {flee} {kill %1}}
```

Замечания:

- **Ленивое выполнение**: ветки `then` и `else` вычисляются только если они выбраны. Переменные внутри невыбранной ветки не подставляются, команды не выполняются.
- **Expression Layer**: поддерживает `$переменные`, `%0/%1/...` capture-параметры, числа, строки, скобки, арифметику `+`, `-`, `*`, `/`, `%`, сравнения `==`, `!=`, `>`, `<`, `>=`, `<=` и базовую boolean-логику `&&`, `||`, `!`, `and`, `or`, `not`.
- Если выражение возвращает не boolean значение, команда выдаст локальную ошибку.
- Если переменная не найдена, она трактуется как пустая строка `""`.
- Если capture-параметр `%N` отсутствует, он трактуется как пустая строка `""` в строковом/equality-контексте. Поэтому guard-условия вроде `%0 != ""` и `%1 != ""` безопасны: отсутствующий аргумент даст `false`, присутствующий непустой аргумент даст `true`.
- Если capture выглядит как число, в числовом выражении он сравнивается как число: `%1 > 10`, `%1 == 0`, `%1 % 2 == 0`.
- Если отсутствующий или пустой capture попал в числовое сравнение, он считается числом `0`. Например, `%0 > 0` без аргументов даст `false`, а `%0 == 0` даст `true`.
- Если непустой нечисловой capture попал в числовое сравнение, это ошибка выражения, а не `false`.
- При ошибке выражения `#if` не выполняет ни `then`, ни `else` и выводит локальную диагностическую ошибку в `main`.
- Внутри веток можно использовать `;`, alias, другие системные команды.

Ограничения текущей версии:
- Не поддерживаются вызовы функций (`len(...)`, `math(...)`), доступ к полям (`a.b`), массивы/индексация (`[1, 2][0]`), ternary `? :` и оператор `in`.
- Не поддерживаются регулярные выражения `=~` и математические функции (`#math`).
...
- **персистентность**: настройки (размер, иконка, режим) и подписки основного тикера сохраняются в профиле и экспортируются в `.tt` файлы.
