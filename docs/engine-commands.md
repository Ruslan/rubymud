...
- **Expression Layer**: поддерживает `$переменные`, числа, строки, операторы `==`, `+`, `-`, `*`, `/` и скобки.
...
- Пока не поддерживаются операторы сравнения `>`, `<`, `>=`, `<=`, `!=`.
- Пока не поддерживаются логические операторы `&&`, `||`, `!`.
- Пока не поддерживаются регулярные выражения `=~` и математические функции (`#math`).
